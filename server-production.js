...
// NOTE: /verify-balance-before-payment endpoint removed. Balance pre-checks are disabled.
...
app.post('/process-payment', async (req, res) => {
    try {
        ...
        // NOTE: Server-side pre-payment wallet balance checks removed.
        // We will attempt the collection request directly and let the payment provider
        // (ioTec) return the definitive outcome. This avoids blocking payments when
        // balance endpoints are flaky or return unexpected responses.
        if (method === 'MobileMoney') {
            console.log('\n🔷 STEP 1: (skipped) Server-side wallet pre-check removed — proceeding to collection request');
        }
...
// NOTE: /check-balance endpoint removed. Balance pre-check endpoints disabled.
